...
export class AuthService {
  ...
  createUserInFirebaseAuthListEmailVerified(email, password, username) {
    ...
    if (isDevMode()) {
      ...
      this.url = `${this.settingsService.getSettings().protocol}${this.settingsService.getSettings().domainName}/user-login-register-slide`;
    }
  ...
  createUserInFirebaseAuthListEmailVerifiedOrder(email, password, username) {
    ...
    if (isDevMode()) {
      ...
      this.url = `${this.settingsService.getSettings().protocol}${this.settingsService.getSettings().domainName}/user-login-register-slide?orderstep=1&login=1`;
    }
    const actionCodeSettings = {
      url: this.url,
      handleCodeInApp: true
